(() => {
    ...
    carousel();
    function carousel() {
        const carouselSlider = document.querySelector(".brands__body");
        const listSlider = document.querySelector(".brands__list");
        const containerWidth = 1180;
        const carouselWidth = containerWidth + 40;
        const speed = 1;
        let sliderCopy;
        console.log(carouselWidth);
        let x = 0;
        let x2 = carouselWidth;
        function cloneSlider() {
            sliderCopy = listSlider.cloneNode(true);
            carouselSlider.append(sliderCopy);
        }
        function moveFirst() {
            x -= speed;
            if (carouselWidth >= -1 * x) listSlider.style.left = x + "px"; else x = carouselWidth;
        }
        function moveSecond() {
            x2 -= speed;
            if (carouselWidth >= -1 * x2) sliderCopy.style.left = x2 + "px"; else x2 = carouselWidth;
        }
        cloneSlider();
        moveSecond();
        setInterval(moveFirst, 25);
        setInterval(moveSecond, 25);
    }
    window["FLS"] = true;
    isWebp();
    menuInit();
