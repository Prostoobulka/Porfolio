...
***************************************************************************** */
    ...
    da.init();
    let btn = document.querySelectorAll(".content-create-ac__item--scale");
    document.addEventListener("click", action);
    function action(e) {
        for (let i = 0; i < btn.length; i++) if (e.target.closest(".content-create-ac__item--scale") && e.target.closest(".content-create-ac__item--scale") === btn[i]) btn[i].classList.toggle("_active"); else btn[i].classList.remove("_active");
    }
    function timer() {
        const timerElement = document.querySelector(".timer");
        if (timerElement) {
            const timerHoursElement = timerElement.querySelector(".timer__hours");
            ...
            function startTimer(deadline) {
                const x = setInterval((function() {
                    ...
                    timerHoursElement.innerHTML = hours;
                    timerMinutesElement.innerHTML = minutes;
                    timerSecondsElement.innerHTML = seconds;
                    if (distance < 0) {
                        clearInterval(x);
            ...
            window.addEventListener("beforeunload", (function(event) {
                localStorage.setItem("deadline", timerElement.getAttribute("data-deadline"));
            }));
            const storedDeadline = localStorage.getItem("deadline");
            if (storedDeadline) startTimer(new Date(storedDeadline).getTime()); else {
                const initialDeadline = new Date("2025-07-08").getTime();
                timerElement.setAttribute("data-deadline", initialDeadline);
                localStorage.setItem("deadline", initialDeadline);
